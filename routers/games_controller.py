...
@router.get("/{game_id}", status_code=status.HTTP_200_OK, response_model=Game)
...
@router.post("/{team_id}", status_code=status.HTTP_201_CREATED, response_model=str)
async def create_game(team_id: str, new_game: Game, player_id: str = Depends(get_current_player)):
    ...
    return f"Game {new_game.game_id} has started."


@router.put("/finish_game", status_code=status.HTTP_200_OK, response_model=str)
async def finish_game(game_to_finish: EndGame, player_id: str = Depends(get_current_player)):
    ...
    return f"Game with id {game_to_finish.game_id} was finished."
    

@router.put("/play_game", status_code=status.HTTP_200_OK, response_model=dict)
