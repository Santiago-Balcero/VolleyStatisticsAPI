...
@router.get("/{team_id}", status_code=status.HTTP_200_OK, response_model=Team)
...
@router.post("/new_team", status_code=status.HTTP_201_CREATED, response_model=str)
async def create_team(new_team: Team, player_id: str = Depends(get_current_player)):
    ...
    return f"Team {new_team.team_name} successfully registered."
...
async def update_team_name(updated_team: UpdatedTeam, player_id: str = Depends(get_current_player)):
    ...
    return f"Team changed it's name to {updated_team.new_team_name}."


@router.delete("/{team_id}", status_code=status.HTTP_200_OK, response_model=str)
